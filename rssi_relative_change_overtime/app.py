...
base = alt.Chart(target_df).properties(width=350, height=350).add_selection(
    selector,
)
...
data_col_text = alt.Chart(target_df).mark_text(dx=20, fontSize=15).encode(
...
sensor_points = alt.Chart(
    sensor_points_loc_df[sensor_points_loc_df.sensor == sel_sensor],
).mark_point(filled=True, size=300, shape='diamond').encode(
    x=alt.X('x', type='quantitative'),
    y=alt.Y('y', type='quantitative'),
...
# Add sensor text
sensor_text = alt.Chart(
    sensor_points_loc_df[sensor_points_loc_df.sensor == sel_sensor],
).mark_text(dx=20, fontSize=15).encode(
    x=alt.X('x', type='quantitative'),
    y=alt.Y('y', type='quantitative'),
    text=alt.Text('sensor:N'),
    color=alt.value('red'),
...
rlt_chng = base.mark_line().encode(
    x=alt.X(
        'date', type='nominal', axis=alt.Axis(labelAngle=0, title='Date'),
    ),
    y=alt.Y(
        'rc',
